...
FOSSIL_SUITE(objcpp_pqueue_tofu_fixture);

FOSSIL_SETUP(objcpp_pqueue_tofu_fixture) {
    // Setup the test fixture
}

FOSSIL_TEARDOWN(objcpp_pqueue_tofu_fixture) {
...
FOSSIL_TEST(objcpp_test_pqueue_create_container_and_destroy) {
    PQueue pqueue("i32");
    ASSUME_ITS_EQUAL_U32(pqueue.size(), 0u);
    // No direct type check; assume construction worked.
}

FOSSIL_TEST(objcpp_test_pqueue_insert_and_remove) {
    PQueue pqueue("i32");
    pqueue.insert("10", 2);
    pqueue.insert("20", 1);
...
FOSSIL_TEST(objcpp_test_pqueue_create_copy) {
    PQueue pqueue1("i32");
    pqueue1.insert("1", 1);
    pqueue1.insert("2", 2);
...
FOSSIL_TEST(objcpp_test_pqueue_create_move) {
    PQueue pqueue1("i32");
    pqueue1.insert("42", 5);
...
FOSSIL_TEST(objcpp_test_pqueue_not_empty_and_is_empty) {
    PQueue pqueue("i32");
...
FOSSIL_TEST(objcpp_test_pqueue_not_cnullptr_and_is_cnullptr) {
...
FOSSIL_TEST(objcpp_test_pqueue_set_front_and_set_back) {
    PQueue pqueue("i32");
    pqueue.insert("100", 1);
    pqueue.insert("200", 2);
...
FOSSIL_TEST(objcpp_test_pqueue_get_front_and_get_back_empty) {
    PQueue pqueue("i32");
    ASSUME_ITS_EQUAL_CSTR(pqueue.get_front().c_str(), "");
    ASSUME_ITS_EQUAL_CSTR(pqueue.get_back().c_str(), "");
}

FOSSIL_TEST(objcpp_test_pqueue_remove_empty) {
    PQueue pqueue("i32");
...
FOSSIL_TEST(objcpp_test_pqueue_size_consistency) {
    PQueue pqueue("i32");
    ASSUME_ITS_EQUAL_U32(pqueue.size(), 0u);
    pqueue.insert("1", 1);
    ASSUME_ITS_EQUAL_U32(pqueue.size(), 1u);
